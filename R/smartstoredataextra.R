...
extract_smartstore_grade_zzim <- function(x1) {
  map2_dfr(x1,names(x1),function(item,n) {
    grade=ifelse(!is.null(item$grade),cleanFun(item$grade),"무등급")
    zzim=ifelse(!is.null(item$zzim),as.numeric(item$zzim),0)
    zzim=ifelse(!is.na(zzim),zzim,0)
    rv=data.frame(myid=n,grade,zzim)
    rv
  })
}
